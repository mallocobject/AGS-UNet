...
class Mlp(nn.Module):
    def __init__(
        ...
        if local_enhence:
            if use_partial:
                self.leconv = PartialConv_1d(
                    hidden_features, hidden_features, forward="split_cat"
                )
            else:
                self.leconv = nn.Conv1d(
                    hidden_features,
                    hidden_features,
                    kernel_size=3,
                    stride=1,
...
class LinearProjection(nn.Module):
    ...
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.0, bias=True) -> None:
        """Linear Projection for generate Q K V vectors
    ...
    def forward(self, x, attn_kv=None):
        ...
        q = (
            self.to_q(x)
            .reshape(B_, N, 1, self.heads, self.inner_dim // self.heads)
            .permute(2, 0, 3, 1, 4)
        )
        kv = (
            self.to_kv(attn_kv)
            .reshape(B_, N_kv, 2, self.heads, self.inner_dim // self.heads)
            .permute(2, 0, 3, 1, 4)
        )
        q = q[0]
        k, v = kv[0], kv[1]
...
class MSAttention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv_proj = LinearProjection(
            dim, num_heads, head_dim, dropout=0, bias=qkv_bias
        )
