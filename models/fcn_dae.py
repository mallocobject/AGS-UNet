...
class FCNDAE1D(nn.Module):
    def __init__(self):
        ...
        self.decoder = nn.Sequential(
            ...
            CNNTransBlock1D(1, 40, kernel_size=16, stride=2),
            CNNTransBlock1D(40, 20, kernel_size=16, stride=2),
            CNNTransBlock1D(20, 20, kernel_size=16, stride=2),
            CNNTransBlock1D(20, 20, kernel_size=16, stride=2),
            CNNTransBlock1D(20, 40, kernel_size=16, stride=2),
    ...
    def forward(self, x):
        ...
        z = self.encoder(x)
        out = self.decoder(z)
        return out.squeeze(1)  # 去掉通道维度，变回 [B, L]


# ========== 测试 ==========
if __name__ == "__main__":
    model = FCNDAE1D()
    x = torch.randn(4, 1024)  # batch=4, 1通道, 长度1024
    y = model(x)
